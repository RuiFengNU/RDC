...
% MATLAB code to solve the provided system of linear equations and plot the results
...
% theta_values = linspace(0.25, 0.75, 3); % Adjust number of theta values as needed
...
% Plotting kappa for different theta values with specified colors
figure;
hold on;
...
for k = 1:length(theta_values)
    ...
    plot(t, kappa(:, k), 'Color', color, 'LineWidth', 1.5, 'DisplayName', ['\theta = ' num2str(theta_values(k))]);
    plot(t, kappa_far(:, k), '--', 'Color', color, 'LineWidth', 1.5, 'DisplayName', ['\theta = ' num2str(theta_values(k)) ' (far)']);
end

hold off;
xlabel('T', 'FontSize', fontSize_label, 'FontName', fontName);
ylabel('$\hat{K}$', 'Interpreter', 'latex', 'FontSize', fontSize_label, 'FontName', fontName);
legend('show', 'Location', 'eastoutside'); % Position the legend outside the plot
set(gca, 'FontSize', fontSize_axis, 'FontName', fontName);
set(gca, 'box', 'on');
ylim([0 0.7]); % Set vertical axis from 0 to 1



% % Plotting kappa for different theta values
% figure;
% hold on;
% 
% % Generate colors based on the number of theta values
% colors = cool(length(theta_values));
% 
% for k = 1:length(theta_values)
%     if theta_values(k) == 0.625
%         plot(t, kappa(:, k), 'r', 'LineWidth', 1.5, 'DisplayName', ['\theta = ' num2str(theta_values(k))]);
%         plot(t, kappa_far(:, k), 'r--', 'LineWidth', 1.5, 'DisplayName', ['\theta = ' num2str(theta_values(k))]);
%     else
%         plot(t, kappa(:, k), 'k', 'LineWidth', 1, 'DisplayName', ['\theta = ' num2str(theta_values(k))]); % black lines for other theta values
%         plot(t, kappa_far(:, k), 'k--', 'LineWidth', 1, 'DisplayName', ['\theta = ' num2str(theta_values(k))]); % black lines for other theta values
%     end
% end
% 
% hold off;
% xlabel('$T$', 'Interpreter', 'latex', 'FontSize', fontSize_label, 'FontWeight', 'bold','FontName', fontName);
