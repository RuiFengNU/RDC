...
for m = 2:M+1
    ...
    for i = 1:n-1
        if abs(x(i)) < theta
            A(i, :) = (C(i, :) + D(i, :));
            b(i) = 2 - (2*sigma_0/p_c) + sum(D(i, :) .* F(:, m-1).');  % F(s_j, t_{m-1})
        else
            A(i, :) = C(i, :);
            b(i) =  - (2*sigma_0/p_c);
        end
    ...
    for i = 1:n-1
        sigma_yy(i, m) = (1/2) * (1/n) * sum(F(:, m).' ./ (x(i) - s)) + (sigma_0/p_c);
    end
...
for i = 1:length(plotIndices)
    plot(x, delta(:, plotIndices(i)),'Color', colors(i,:),'LineWidth', 2);
end
hold off;
xlabel('X', 'FontSize', fontSize_label, 'FontWeight', 'bold');
ylabel('\Delta(X, T)', 'FontSize', fontSize_label, 'FontWeight', 'bold');
...
hold on;
for i = 1:length(plotIndices)
    plot(x, sigma_yy(:, plotIndices(i)),'Color', colors(i,:), 'LineWidth', 2);
end
hold off;
xlabel('X', 'FontSize', fontSize_label, 'FontWeight', 'bold');
ylabel('P(X, T)', 'FontSize', fontSize_label, 'FontWeight', 'bold');
